...
export default class VerifyUserMiddleware{
  static isPasswordAndUserMatch(req, res, next){
    ...
    db.collection("User").findByEmail(req.body.email).then( user => {
      if(!user.length()){
        res.sendStatus(404)
      }else{
        let user = user[0]
        ...
        if(hash == password){
          ...
          return next()
            ...
            return res.sendStatus(400).send({errors: ['Invalid email or password']});
        }
      }
    })
  }
